...
try  {
    var initConnection = function initConnection(qCon, connection) {
        var i, l, g, r;
        qCon(connection, {
    ...
    if ((typeof(require) !== 'undefined') && (typeof(module) !== 'undefined')) {
        ...
        freeport(function(err, port) {
            Helios._wssPort = port;
            var wss = new WebSocketServer({port: port});

            wss.on('connection', function (ws) {
                initConnection(qCon, ws);
            ...
            deferred.resolve(Helios);
        });

        module.exports = deferred.promise;
    } else {
        initConnection(Q_COMM, this);
